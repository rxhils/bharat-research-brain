// Maven answer pipeline, extracted verbatim from app/api/ask/route.ts so multiple endpoints
// (/api/ask, /api/mobile/ask) share ONE brain. This is a pure move - no logic, prompt, routing,
// guardrail or answer-shape change. The only difference vs the old POST handler is that each
// path returns the MavenAnswer object directly instead of wrapping it in NextResponse.json();
// the HTTP concerns (body parsing, empty-query 400, error envelope) live in the route handlers.
...
export const MAX_QUERY_CHARS = 2000; // hard input cap - a question is never this long, an abuse payload is
...
/**
 * The full Maven answer pipeline. Given a (already length-capped, non-empty) query and the raw
 * client conversationContext, returns the MavenAnswer. Identical behavior to the previous inline
 * /api/ask handler - only the NextResponse.json() wrapper is removed so multiple routes can reuse it.
 */
export async function answerQuestion(query: string, conversationContext: unknown): Promise<MavenAnswer> {
  const convo = buildConversationState(conversationContext);

  const { answerType, disclaimerLevel } = routeAnswerType(query);
  if (answerType === "greeting") return greeting(query);
  ...
  if (answerType === "unsafe_advice") return REFUSAL;
  ...
  if (lastTurnType === "unsafe_advice" && looksLikeBareFollowUp(query)) return REFUSAL;
  ...
  if (convo.lastAnswer && isMoversCorrection(query)) {
    ...
    if (routed.answerType !== "unsafe_advice" && routed.answerType !== "out_of_scope" && routed.answerType !== "greeting") {
      ...
      return enforceFollowUpChips(answer);
    }
  }
  ...
  if (followUp.isFollowUp && followUp.confidence !== "low" && convo.lastAnswer) {
    ...
    if (followUp.followUpType === "explain_leaderboard") {
      ...
      if (explained) {
        if (transformedAnswerAssertsAdvice(explained)) return REFUSAL;
        return enforceFollowUpChips(explained);
      }
    ...
    if (isTransformationMode(mode)) {
      ...
      if (transformedAnswerAssertsAdvice(transformed)) return REFUSAL;
      return enforceFollowUpChips(transformed);
    }
    ...
    if (routed.answerType === "unsafe_advice") return REFUSAL;
  ...
  if (answerType === "out_of_scope") {
    ...
    if (looksLikeBareFollowUp(query)) return noContextCard();
    return outOfScope(query);
  }

  return await research(query, answerType, disclaimerLevel);
}
