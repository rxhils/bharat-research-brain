...
export function planResearch(query: string, intent: Intent): ResearchPlan {
  ...
  switch (intent) {
    case "single_stock": {
      const rs = resolveStock(query);
      const req = ["stock", "snapshots", "announcements", "sectors", "indices"];
      const g = rs ? sectorGroup(rs.sector) : "other";
      if (g === "banks") req.push("gsec", "fiidii");
      else if (g === "energy") req.push("crude", "usdinr");
      else if (g === "exporter") req.push("usdinr");
      const name = rs?.companyName ?? topic;
      return {
        intent, topic: name, requiresLiveData: true, requiredData: req, requiredCharts: ["stock_line", "valuation"],
        searchQueries: [
          `${name} latest exchange filing NSE today`,
          `${name} results investor presentation India`,
          `${name} stock news today why moving India`,
        ],
      };
    }
    case "market_summary":
    ...
    case "sector_impact":
      return mk(["sectors", "indices", "crude", "usdinr", "gsec", "fiidii", "macro"], ["sector_bar"],
        [`${topic} India market today`, `${topic} impact Indian stocks`]);
    case "stock_comparison":
      return mk(["stocks", "snapshots", "announcements", "sectors"], ["stock_compare", "valuation"],
        [`${topic} comparison India`, `${topic} latest quarterly results`]);
    case "macro_impact":
      return mk(["crude", "usdinr", "gsec", "macro"], ["macro_line"],
