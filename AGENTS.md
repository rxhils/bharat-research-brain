...
- **2026-05-29 · Phase 4 Chunk 4.10 · VCP wired into the composite ranking score — RESOLVED.** The agent + table were reworked to the authoritative `stock_vcp_signals` design (composite `vcp_score` from trend / contraction quality / volume dry-up / pivot proximity / relative strength). `RankingAgent.run_all` now loads `vcp_repo.fetch_latest()` and `score_technical` adds +10 (vcp_score ≥ 60) / +5 (≥ 40) when `vcp_detected`. Done in this chunk per the operator's STEP 5 instruction (supersedes the 2026-05-28 storage-first note). The old `vcp_signals` table/model and `vcp_indicators.py` were removed.
- **2026-05-29 · Phase 4 Chunk 4.10 · VCP signal not written to vault stock-note frontmatter.** Per the frontmatter contract, a `vcp` boolean / `vcp_pivot` field could surface on `01_Stocks/<TICKER>.md`. Deferred to the vault-writer wiring chunk; no schema change made here. Status: open.
- **2026-05-29 · Phase 4 Chunk 4.10 · Relative strength uses a large-cap proxy, not a real Nifty 50 series.** `vcp_repo.load_market_proxy` prefers a real index row (`nse_symbol` in NIFTY/^NSEI/…) but falls back to an equal-weight average of the top-10 large-caps' `adj_close` per date, since `prices_eod_adjusted` has no Nifty-50 index series. Load a true Nifty 50 close series for accurate RS scoring. Status: open.
- **2026-05-29 · Phase 4 Chunk 4.10 · Volume dry-up reads `adj_volume`, not raw `prices_eod` volume.** The spec said source volume from `prices_eod`, but the agent uses `prices_eod_adjusted.adj_volume` so OHLC and volume come from one already-adjusted series (CLAUDE.md §4: adjust before any technical calc). Documented deviation; revisit only if split-adjusted volume proves misleading for the dry-up test. Status: noted.
- **2026-05-29 · Phase 4 Chunk 4.10 · `find_contractions` returns 0 on EVERY real stock — VCP detection is functionally inert.** Live `vcp run` (507 rows / 503 scored): `contraction_count > 0` for **0** stocks, so the `count ≥ 2` detection gate never passes and `vcp_detected = 0` universally → the +10/+5 ranking bonus currently affects **0 stocks** (ranking distribution unchanged by VCP). Every other component works live: trend_score ≥ 75 → 203, vcp_score ≥ 40 → 106, volume_dryup → 127, pivot_proximity reaches 100. The 19 pure unit tests pass on synthetic `_zigzag` series (clean alternating swing pivots); real adjusted series are noisy and the swing-pivot + "each leg ≥10% shallower than prior, monotonic across all legs" rule collapses to 0. Needs a detector rework (tolerance / smoothing / swing-window tuning) validated against real OHLC, not just synthetic fixtures. **NOT fixed here — surfaced during verification-only run (SCOPE LOCK).** Status: open, high priority.
- **2026-05-29 · Phase 4 Chunk 4.10 · `load_market_proxy` returned EMPTY live (`vcp.no_market_proxy` warning).** Stronger than the proxy follow-up above: neither a NIFTY index row nor the top-10 large-cap fallback produced a series, so `relative_strength` fell to its neutral default for all stocks. Confirms a real Nifty-50 adjusted-close series must be ingested before RS scoring is meaningful. Status: open (relates to the RS-proxy follow-up).
- **2026-06-14 · Chunk 5.2b Task 3 · KNOWN LIMITATION — `macro_signals_historical` warmup pollution.** The breadth backfill emits a row for EVERY trading date, so the two lookback-dependent indicators carry placeholder values during their warmup: `pct_above_ema200` first ~200 rows (from 2021-05-26) are placeholder `0` (not skipped during the EMA200 warmup); `new_high_low_ratio` first ~252 rows are placeholder `1` during the 52-week-extreme warmup. HARMLESS for backtests starting 2024+ (warmup long past; the macro reconstructor reads the most-recent row ≤ D, always a post-warmup value). WOULD MATTER for backtests over 2021-2022 windows. Fix when extending walk-forward to Window 1 (2019-21): skip rows where the indicator's lookback isn't yet valid, and DELETE the existing placeholder rows. Status: open, deferred to Week 3 Window 1.
- **2026-06-14 · Chunk 5.2b Week 2 (FUTURE) · Turnover controls to apply to ALL composite-backtest variants.** When Week 2 runs, add to the variant testing: (1) minimum 30-day cooldown between exiting and re-entering the same stock (prevents churn on noisy signals); (2) max 30% of positions may change per rebalance cycle (forces existing positions time to play out); (3) track + report average holding period actually achieved, turnover rate per rebalance, re-entered stocks (and how soon), total trade count. Healthy = 500-2000 total trades over the 2-year window; >5000 is over-trading; <200 may mean the signal finds nothing. Status: not started (Week 2 is out of scope until Week 1 closes).
