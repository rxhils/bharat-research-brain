...
def run(date: str, *, story: dict, hooks: dict, script_edited: dict,
        viral_fit: dict, template: dict | None = None,
        variation: dict | None = None) -> dict:
    accent = (variation or {}).get("accent_color") or (template or {}).get("accent_color") or "#22D3EE"
    ...
    whyw = str(story.get("why_it_matters", "")).split()
    why1 = " ".join(whyw[:7]) or "One sector moved the whole market"
    why2 = " ".join(whyw[7:14]) or "When it drops, the index follows"
    take = " ".join(str(story.get("investor_takeaway", "")).split()[:8]) or "Watch the sectors, not just the index"

    # 9 fast micro-scenes (avg ~1.8s), total ~16s
    scenes = [
        {"kind": "hook", "duration": 1.2, "title": on_hook, "on_screen": on_hook,
        ...
        {"kind": "stat", "duration": 2.2, "label": label, "value": value, "suffix": suffix, "sub": sub,
         "on_screen": f"{label} {value}{suffix}", "visual": "Index stat card + number counter",
         "motion": "slide-up + count-up", "transition_out": "slide", "sfx": "tick", "asset": "asset_bg_dark"},
        {"kind": "chips", "duration": 1.8, "chips": sectors, "on_screen": " / ".join(sectors),
         "visual": "Sector chips stagger in", "motion": "stagger-pop", "transition_out": "wipe",
         "sfx": "tick", "asset": "asset_bg_panel"},
        {"kind": "reason", "duration": 2.0, "text": why1, "on_screen": why1,
         "visual": "Key reason kinetic text", "motion": "word rise", "transition_out": "fade",
         "sfx": "whoosh", "asset": "asset_bg_dark"},
        {"kind": "chart", "duration": 2.2, "points": [8, 7.2, 7.4, 6, 5.2, 3.4, 2.7, 2.4],
         "on_screen": "", "visual": "Mini index line draws down", "motion": "line-draw + dot",
         "transition_out": "flash", "sfx": "whoosh", "asset": "asset_bg_dark"},
        {"kind": "reason", "duration": 1.8, "text": why2, "on_screen": why2,
         "visual": "Second reason kinetic text", "motion": "word rise", "transition_out": "fade",
         "sfx": "", "asset": "asset_bg_panel"},
        {"kind": "reason", "duration": 1.8, "text": take, "on_screen": take,
         "visual": "Takeaway kinetic text", "motion": "word rise", "transition_out": "fade",
         "sfx": "", "asset": "asset_bg_dark"},
        {"kind": "outro", "duration": 2.2, "text": "Understand the Indian market with Maven",
         "on_screen": "Maven · trymaven.in", "visual": "Maven end card", "motion": "scale-in",
         "transition_out": "", "sfx": "", "asset": "asset_bg_end"},
    ]
    # assign start times + variation accent
    t = 0.0
    ...
    payload = {"date": date, "total_duration": round(t, 1), "scene_count": len(scenes),
               "fps": 30, "accent_color": accent,
               "template": (template or {}).get("selected_template"),
               "variation_id": (variation or {}).get("variation_id"),
               "scenes": scenes}
