...
export default function RunPage() {
  ...
  // refetch node statuses as live events arrive
  ...
  const onAction = useCallback(async (label: string, fn: () => Promise<unknown>) => {
    ...
    try {
      ...
      setToast(`${label}: ${e?.data?.problems?.join(", ") || "failed"}`);
    }
      ...
      <div className="px-6 py-3 border-b border-line flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[11px] text-ink-faint capitalize">{job?.run_type} · {job?.market_status}</div>
          </div>
          {job && <StatusBadge status={job.status} glow />}
        </div>
        {job?.instagram_post_url && (
          <a href={job.instagram_post_url} target="_blank" rel="noreferrer" className="btn btn-primary">View IG post</a>
        )}
      </div>
